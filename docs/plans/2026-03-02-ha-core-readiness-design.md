...
## Workstream 1: Code Style Alignment [DONE]
...
### 1.3 Pyright strict compliance [DONE]
...
## Workstream 2: Bronze Gaps [DONE]

### 2.1 `has-entity-name` [DONE]
...
## Workstream 3: Silver Gaps [DONE]

### 3.1 `reauthentication-flow` [DONE]
...
### 3.4 `config-entry-unloading` [DONE]
...
## Workstream 4: Gold Easy Wins [DONE]

### 4.1 `reconfiguration-flow` [DONE]
...
### 4.4 `entity-translations` and `icon-translations` [DONE]
...
### 4.7 Config entry versioning [DONE]
...
## Workstream 5: Test Infrastructure Rewrite [DONE]

Big-bang rewrite of the test infrastructure. This is the largest single item.

### 5.1 Add `pytest-homeassistant-custom-component` [DONE]
...
### 5.4 Config flow test coverage [DONE]
...
Already at 99% coverage (645 lines, 36 tests).

### 5.5 Enforce coverage threshold [DONE]

Add `--cov-fail-under=95` to pytest configuration.

Final result: 521 tests, 96.81% coverage (76 lines uncovered).
Added `.coveragerc` to exclude `TYPE_CHECKING` blocks.
Added `--cov-fail-under=95` to Makefile test target.

### 5.6 Contract tests [DONE]
