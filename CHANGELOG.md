...
### Fixed
- Compatibility with the Home Assistant 2026.8 device registry, where a device belongs to a single config entry: the entity cache now reads `DeviceEntry.config_entry_id` instead of the deprecated `config_entries` set (removal planned for 2027.8), falling back to the set on older HA versions. The rest of the integration was audited against the 2026.8 changes and needed no other updates (#269)
...
- A post-login HTTP 401/403 from the controller (login accepted but the request refused -- typically a wrong Site value, an account lacking site permission, or a 2FA-restricted account) is now surfaced as a distinct, actionable error pointing at the Site/permissions instead of a misleading "Cannot connect". At runtime this no longer triggers a credential reauth loop and backs off between retries (#231)
- Added help text to the Site field in the setup and reconfigure forms explaining it needs the internal site ID (usually `default`), not the site's display name -- which previously caused a confusing connection failure on multi-site controllers (#231)
- The WebSocket subscription now sends a result message before the first event, so the card's subscribe promise actually resolves. Previously the card could never store its unsubscribe callback, kept HTTP polling running alongside WebSocket pushes, and leaked a server-side coordinator listener per re-subscribe
- A rate-limited controller (HTTP 429) during setup validation is now reported as a connection problem instead of "invalid credentials", which misled users into retrying correct passwords against a lockout
- An unknown `svg_theme` query value on the SVG endpoint now falls back to the default theme instead of returning HTTP 500
- Unloading the last config entry now unsubscribes the entity/device registry listeners instead of leaking them until Home Assistant restarts
